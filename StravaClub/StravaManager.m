...
{    
    ...
    // http://app.strava.com/api/v1/streams/9968702?streams[]=latlng,distance,altitude
...
{        
    ...
    [StravaManager stravaAPIRequest:(NSString*)urlString withBlock:^(id json) {

          NSDictionary *jsonDict = (NSDictionary*)json;
                    
          if ([delegate respondsToSelector:@selector(rideID:rideStreamsDidLoad:)]) {
              dispatch_sync(dispatch_get_main_queue(), ^{ 
                  [delegate rideID:rideID rideStreamsDidLoad:jsonDict];  
              });
          }
      }];
}
